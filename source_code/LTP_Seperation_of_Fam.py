...
for record in SeqIO.parse(LTP_path,"fasta"):
   ...
   # print(len(record_description))
    ...
    if(len(record_description) ==  8):
        ...
        #we want to use  "a" for file opener becuause we want to append new information to it...
        with open(Fam_path + Family + '.fa',"a") as Family_handle:
            SeqIO.write(record, Family_handle, "fasta")
        with open(Genera_path + Genera + '.fa', "a") as Genera_handle:
            SeqIO.write(record, Genera_handle, "fasta")
        with open(Species_path + Species + '.fa', "a") as Species_handle:
